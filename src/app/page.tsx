      ...
      <section className="py-12 sm:py-20 bg-slate-50/70 border-y border-slate-200">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
            ...
            {SAH_CATEGORIES.slice(0, 4).map((cat) => (
              ...
              >
                <div>
                  ...
                  <div className="relative h-28 sm:h-44 overflow-hidden bg-slate-100/60 border-b border-slate-200">
                    <Image
                      src={cat.image}
                    ...
                    {/* SKU Code Pill */}
                    <div className="absolute top-2 left-2 sm:top-3 sm:left-3 bg-[#0B1F3A] text-white text-[9px] sm:text-[10px] font-mono font-bold px-2 py-0.5 sm:px-2.5 sm:py-1 rounded">
                      {cat.codePrefix}
                    </div>
                  </div>
                  ...
                  <div className="p-3 sm:p-5 space-y-1 sm:space-y-2">
                    <h3 className="text-xs sm:text-base font-bold text-[#0B1F3A] group-hover:text-[#9A7B1C] transition-colors leading-snug line-clamp-1 sm:line-clamp-none">
                      {cat.name}
                    </h3>
          ...
          <div className="mt-8 sm:mt-10 text-center">
            <Link
              href="/products"
              className="inline-flex items-center space-x-2 px-6 py-2.5 sm:px-7 sm:py-3 bg-[#0B1F3A] text-white text-xs font-bold uppercase tracking-wider rounded-lg hover:bg-[#1E293B] transition-all group shadow-2xs"
            >
      ...
      <section className="py-12 sm:py-20 bg-[#F8FAFC] border-b border-[#E2E8F0]">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 sm:mb-12 space-y-3 md:space-y-0 pb-4 sm:pb-6 border-b border-[#E2E8F0]">
            ...
            <Link
              href="/products"
              className="text-xs font-bold text-[#0B1F3A] hover:text-[#9A7B1C] uppercase tracking-wider flex items-center space-x-1.5 transition-colors group self-start md:self-auto"
            >
            ...
            {featuredProducts.map((prod) => (
              ...
              >
                <div>
                  ...
                  {/* Body Details */}
                  <div className="p-3 sm:p-5 space-y-1 sm:space-y-2">
                    <span className="text-[10px] sm:text-[11px] font-semibold text-[#8C6B1B] uppercase tracking-wider block">
                      {prod.categoryName}
                    </span>
                    ...
                    {/* Finishes Pills */}
                    {prod.finishes && (
                      <div className="mt-2 sm:mt-3 flex flex-wrap items-center gap-1 sm:gap-1.5 pt-1">
                        {prod.finishes.slice(0, 4).map((f) => {
                          ...
                          return swatchClass ? (
                            <span
                              key={f}
                              title={f}
                              className={`w-3.5 h-3.5 sm:w-4 sm:h-4 rounded-full border shadow-2xs inline-block transition-transform hover:scale-125 cursor-pointer ${swatchClass}`}
                            />
                          ) : (
                            <span
                              key={f}
                              className="text-[9px] sm:text-[10px] bg-[#F8FAFC] border border-[#E2E8F0] text-[#334155] px-1.5 sm:px-2.5 py-0.5 rounded font-medium"
                            >
                              {f}
                            </span>
                          );
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-12 items-center">
            ...
            <div className="lg:col-span-6">
              <div className="relative h-[250px] sm:h-[420px] rounded-2xl overflow-hidden shadow-xs border border-[#E2E8F0] bg-[#0F172A] p-2 sm:p-4 flex items-center justify-center">
                <Image
                ...
                <div className="absolute bottom-3 left-3 right-3 sm:bottom-6 sm:left-6 sm:right-6 bg-[#0B1F3A]/95 backdrop-blur-sm text-white p-3 sm:p-5 rounded-xl flex items-center justify-between shadow-md border border-white/10">
                  <div>
                    <div className="text-sm sm:text-lg font-bold text-[#D4AF37]">Finetek</div>
                    <div className="text-[10px] sm:text-xs text-slate-300 font-medium">Branch • Raipur</div>
                  </div>
                  <div className="h-6 sm:h-8 w-px bg-slate-700" />
                  <div className="text-right">
                    <div className="text-sm sm:text-lg font-bold text-white">86+ SKUs</div>
                    <div className="text-[10px] sm:text-xs text-slate-300 font-medium">Ready Raipur Stock</div>
                  </div>
                </div>
              </div>
            </div>
